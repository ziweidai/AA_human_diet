...
NegAANames={'Tryptophan'};
PosAANames={'Methionine'};
[~,NegAAPos_NHANES]=ismember(NegAANames,AANames_NHANES);
[~,PosAAPos_NHANES]=ismember(PosAANames,AANames_NHANES);
nq=5;
ratios=zeros(18,nq);
label_diabetes=DiseaseScores(:,2);
goodpos=find(~isnan(label_diabetes)); %Indice for individuals with BMI record
label_diabetes=label_diabetes(goodpos);
p_Chi2_AA_Diabetes=zeros(1,18);
for i=1:18
    ...
    for k=1:nq
        ...
        ratios(i,k)=sum(label_diabetes(qpos)==1)/length(qpos);
    end
    [~,~,p_Chi2_AA_Diabetes(i)]=crosstab(label_diabetes,label_quantile);
end

figure;
subplot(2,1,1);
heatmap(ratios(NegAAPos_NHANES,:),1:5,NegAANames,[],'MinColorValue',0.09,...
    'MaxColorValue',0.13,'ShowAllTicks',true,'GridLine',':');
title('Negative association');
subplot(2,1,2);
heatmap(ratios(PosAAPos_NHANES,:),1:5,PosAANames,[],'MinColorValue',0.09,...
    'MaxColorValue',0.13,'ShowAllTicks',true,'GridLine',':');
...
for i=1:2
    ...
    [~,~,p]=crosstab(label_diabetes,label_quantile);
    subplot(1,2,i);
...
for i=1:10
    ...
    for j=1:5
        ratios(i,j)=sum(label_diabetes(label_quantile==j))/sum(label_quantile==j);
    end
    [~,~,p(i)]=crosstab(label_diabetes,label_quantile);
end
...
cmap_now = brewermap(100,'RdYlGn');
colormap(cmap_now(75:-1:26,:));
heatmap_cluster(ratios,DietNames,1:5,[0.08 0.13]);
xlabel(sprintf('Quantile of deviation\n from Pareto surface'));
title(sprintf('Diabetes prevalence vs deviation from Pareto surface'));
xtickangle(0);
